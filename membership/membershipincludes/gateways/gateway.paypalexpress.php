...
class paypalexpress extends M_Gateway {
	...
	function single_sub_button($pricing, $subscription, $user_id, $norepeat = false) {
		...
		if (get_option( $this->gateway . "_paypal_status" ) == 'live') {
			$form .= '<form action="'.site_url().'/payment" method="post">';
		} else {
			$form .= '<form action="'.site_url().'/payment" method="post">';
		}
		...
		$form .= '<input type="hidden" name="item_name" value="' . strip_tags($subscription->sub_name()) . '">';
		...
		$form .= '<input type="hidden" id="monthlypay" name="a3" value="' . apply_filters('membership_amount_' . $M_options['paymentcurrency'], number_format($pricing[0]['amount'], 2)) . '">';
		$form .= '<input type="hidden" id="dayperiod" name="p3" value="' . $pricing[0]['period'] . '">';
		$form .= '<input type="hidden" id="payunit" name="t3" value="' . strtoupper($pricing[0]['unit']) . '"> <!-- Set recurring payments until canceled. -->';
		...
		$form .= '<!-- Display the payment button. --> <input type="submit" name="submit" value="Continue">';

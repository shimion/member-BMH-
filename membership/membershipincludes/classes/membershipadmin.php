...
if(!class_exists('membershipadmin')) {
	class membershipadmin {
			...
			if($action == 'updateoptions') {
				...
				switch($tab) {
					...
					case 'users':			$wp_user_search = new WP_User_Query( array( 'role' => 'administrator' ) );
											...
											break;
					case 'emails':			$M_options['sendername'] = (isset($_POST['sendername'])) ? $_POST['sendername'] : get_bloginfo();
											$M_options['senderemail'] = (isset($_POST['senderemail'])) ? $_POST['senderemail'] : get_bloginfo('admin_email');
											$M_options['messagemail'] = (isset($_POST['messagemail'])) ? $_POST['messagemail'] : 'Username: %username% <br /> Password: %username%  <br />%login_url%';
											break;
				...
				switch($action) {
					...
					case 'createregistrationcompletedpage':
													...
													$content = '<div class="successmsgbutton">
													<p class="successmsg">' . __('Thank you for adding your farm to BoardMyHorseNow! You may manage your listing  by clicking on the My Account button below.','membership') . '</p>
													<a class="paa_fieldaddlistion" href="'.site_url("account").'">My Account</a></div>';
		...
		function show_emails_options() {
			global $action, $page, $M_options;
			$messages = array();
			$messages[1] = __('Your options have been updated.','membership');
		?>
			<div class="icon32" id="icon-options-general"><br></div>
			<h2><?php _e('Email Options','membership'); ?></h2>
			<div id="poststuff" class="metabox-holder m-settings">
				<form action='' method='post'>
					<input type='hidden' name='page' value='<?php echo $page; ?>' />
					<input type='hidden' name='action' value='updateoptions' />
					...
					?>
					<div class="postbox">
						<h3 class="hndle" style='cursor:auto;'><span><?php _e('Email Sender Options','membership'); ?></span></h3>
						<div class="inside">
							<p class='description'><?php _e('This is the email id used in the sender email address.','membership'); ?></p>
							<table class="form-table">
							<tbody>
								<tr valign="top">
									<th scope="row"><?php _e('Sender Name','membership'); ?></th>
									<td>
												
										  <?php
										  	$sendername = $M_options['sendername'];
										    $sendername = empty($sendername) ? get_bloginfo()  : $sendername;
											$sendername = apply_filters('membership_email_notification_sendername', $sendername);
										  ?>
										  <input type="text" name="sendername" value="<?php echo $sendername; ?>" size="40"/>
									</td>
								</tr>
								<tr valign="top">
									<th scope="row"><?php _e('Sender Email','membership'); ?></th>
									<td>
												
										  <?php
										  	$senderemail = $M_options['senderemail'];
										    $senderemail = empty($senderemail) ? get_bloginfo('admin_email')  : $senderemail;
											$senderemail = apply_filters('membership_email_notification_senderemail', $senderemail);
										  ?>
										  <input type="text" name="senderemail" value="<?php echo $senderemail; ?>" size="40"/>
									</td>
								</tr>	
								<tr>
									<th scope="row" colspan="2"><?php _e('Notification Mail Message','membership'); ?></th>
											
								</tr>
								<tr>
									<td colspan="2">
										  <?php
											$editor_id = 'messagemail';
										  	$content = $M_options['messagemail'];
										    $content = empty($content) ? 'Username: %username% <br />Password: %username%  <br />%login_url%' : $content;
						...
						<?php	
						echo '<p class="description">' . __( 'Some dynamic tags can be included in your email template :', 'membership' ) . '</p>
						<ul>
						<li>' . __( '<strong>%username%</strong> : will be replaced with the username.', 'membership' ) . '</li>
						<li>' . __( '<strong>%password%</strong> : will be replaced with the username.', 'membership' ) . '</li>
						<li>' . __( '<strong>%blog_url%</strong> : will be replaced with your blog URL.', 'membership' ) . '</li>
						<li>' . __( '<strong>%home_url%</strong> : will be replaced with your home URL.', 'membership' ) . '</li>
						<li>' . __( '<strong>%login_url%</strong> : will be replaced with your login URL.', 'membership' ) . '</li>
						<li>' . __( '<strong>%blog_name%</strong> : will be replaced with your blog name.', 'membership' ) . '</li>
						<li>' . __( '<strong>%blog_description%</strong> : will be replaced with your blog description.', 'membership' ) . '</li>
						<li>' . __( '<strong>%admin_email%</strong> : will be replaced with admin email.', 'membership' ) . '</li>
						<li>' . __( '<strong>%date%</strong> : will be replaced with current date, as formatted in <a href="options-general.php">general options</a>.', 'membership' ) . '</li>
						<li>' . __( '<strong>%time%</strong> : will be replaced with current time, as formatted in <a href="options-general.php">general options</a>.', 'membership' ) . '</li>
					</ul>';
						?>
						</div>
						...
						<?php
							do_action( 'membership_emailoptions_page' );
						?>
						<p class="submit">
							<input type="submit" name="Submit" class="button-primary" value="<?php esc_attr_e('Save Changes','membership'); ?>" />
						</p>
		...
		function handle_options_panel() {
			...
			<div class='wrap nosubsub'>
				<?php
					...
					$menus['emails'] = __('Emails', 'membership');
				...
				switch($tab) {
					...
					case 'downloads':		$this->show_downloads_options();
											break;
					case 'emails':			$this->show_emails_options();
											break;

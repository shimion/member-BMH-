	...
	class membershippublic {
		...
		function new_user_notification_mail($userid,$userpassword) {
			...
			foreach ( $to_replace as $tag => $var ) {
				$this->emailoptions['template'] = str_replace( '%' . $tag . '%', $var, $this->emailoptions['template'] );
			}
			add_filter('wp_mail_content_type',create_function('', 'return "text/html"; '));
			$headers = apply_filters( 'memebership_email_headers', "From: ".$this->emailoptions['from_name']." <".$this->emailoptions['from_email'].">  \r\n");
			
			/*wp_mail( $users->user_email, 'Welcome To Board My Horse Now!', $this->emailoptions['template'], $headers);*/
		}
